"""Milestone 1 Empirical Stress Test Suite
...
class TestM1ExactGreetingVerification(BaseSeleniumTest):
    ...
    def test_greeting_rendered_in_modal_dom(self):
        # Open love letter modal so stage-letter is visible
...
class TestM1ConfettiStress(BaseSeleniumTest):
    ...
    def test_confetti_particle_array_under_rapid_fire(self):
        # Empirically verify that even with 15 rapid consecutive triggers, active particles never exceed 250
        res = self.driver.execute_script("""
            return new Promise((resolve) => {
                let maxParticlesInFrame = 0;
                let currentFrameParticles = 0;
                ...
                CanvasRenderingContext2D.prototype.clearRect = function(...args) {
                    if (currentFrameParticles > maxParticlesInFrame) {
                        maxParticlesInFrame = currentFrameParticles;
                    }
                ...
                // Rapidly fire celebration confetti 15 times
                for (let i = 0; i < 15; i++) {
                    window.triggerCelebrationConfetti();
                }
                
                setTimeout(() => {
                    CanvasRenderingContext2D.prototype.save = origSave;
                    CanvasRenderingContext2D.prototype.clearRect = origClearRect;
                    resolve({ maxParticles: maxParticlesInFrame });
                }, 1200);
            });
        """)
        max_p = res.get('maxParticles', 0)
        self.assertGreater(max_p, 0, 'Confetti must spawn particles')
        self.assertLessEqual(max_p, 250, f'Active particles ({max_p}) exceeded MAX_CONCURRENT_PARTICLES (250)')
